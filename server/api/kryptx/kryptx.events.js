/**
 * Kryptx model events
 */

'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});

var _events = require('events');

var _kryptx = require('./kryptx.model');

var _kryptx2 = _interopRequireDefault(_kryptx);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var KryptxEvents = new _events.EventEmitter();

// Set max event listeners (0 == unlimited)
KryptxEvents.setMaxListeners(0);

// Model events
var events = {
  'save': 'save',
  'remove': 'remove'
};

// Register the event emitter to the model events
for (var e in events) {
  var event = events[e];
  _kryptx2.default.schema.post(e, emitEvent(event));
}

function emitEvent(event) {
  return function (doc) {
    KryptxEvents.emit(event + ':' + doc._id, doc);
    KryptxEvents.emit(event, doc);
  };
}

exports.default = KryptxEvents;
//# sourceMappingURL=kryptx.events.js.map
